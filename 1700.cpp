...
int dfs(int start, vector<int> &dp, vector<vector<int>> &adj_scc,
    ...
    while (!s.empty()) {
        ...
        color[u] = 1; // mark as being visited (gray)

        bool finished = true;
        for (int v : adj_scc[u]) {
            if (color[v] == 0) { // if unvisited (white)
                s.push(v);
                finished = false;
            }
        }
        ...
        if (finished) {
        if (color[u] == 1) { // check if the node is being visited (gray)
            dp[u] = 0;
            for (int v : adj_scc[u]) {
                if (color[v] >= 0) { // if visited (gray or black)
                    dp[u] = max(dp[u], 1 + dp[v]);
                }
            }
            color[u] = 2; // mark as completely visited (black)
        }
